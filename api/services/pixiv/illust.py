...
async def get_illust_rank_handler(mode, date, offset):
    try:
        if auth_error := await _ensure_pixiv_auth():
            return auth_error

        json_result = await aapi.illust_ranking(mode=mode, date=date, offset=offset)
        ...
        if final_error := json_result.get("error"):
            ...
            error_message = (
                final_error.get("user_message")
                or "Error fetching rankings or API error."
            )
            return error_response(error_message, 404)
...
async def search_illust_handler(
    word, search_target="partial_match_for_tags", sort="date_desc", offset=None
):
    try:
        if auth_error := await _ensure_pixiv_auth():
            return auth_error

        json_result = await aapi.search_illust(
            word=word,
        ...
        if final_error := json_result.get("error"):
            ...
            error_message = (
                final_error.get("user_message")
                or "Error performing illustration search or API error."
            )
            return error_response(error_message, 404)
        ...
        logger.error(
            f"An unexpected error occurred in search handler: {e}", exc_info=True
        )
        return error_response("An internal server error occurred.", 500)
